<x-app-layout>
    ...
    <div class="bg-surface-container-lowest rounded-xl shadow-[0_20px_50px_rgba(0,0,0,0.3)] overflow-hidden">
        ...
        <div class="p-8 pb-0 text-center">
            ...
            <h1 class="text-3xl font-extrabold text-on-surface tracking-tight mb-2">Modifier une agence</h1>
            <p class="text-on-surface-variant body-md max-w-md mx-auto">
                Remplissez les informations pour modifier une agence .
            </p>
        </div>
        <!-- Form Start -->
        <form action="{{ route('modifier_agence',['id_agence'=>$agence->id]) }}" class="p-8 space-y-8" method="POST" >@csrf @method('PUT')
            <!-- Section 1: Business Info -->
            <section class="space-y-6">
                <header class="flex items-center gap-2">

                    <h2 class="text-sm font-bold uppercase tracking-wider text-on-surface">Informations de l'agence
                    </h2>
                </header>
                <div class="space-y-4">
                    <!-- Nom de l'agence -->
                    <div class="relative">
                        <label class="block text-xs font-bold text-on-surface-variant mb-1 ml-1" for="company_name">
                            Nom de l'agence <span class="text-primary">*</span>
                        </label>
                        <div class="relative group">
                            
                            <input class="sunken-input w-full pl-12 pr-4 py-3.5 rounded-lg text-on-surface font-medium"
                                id="libelle" name="nom" placeholder="Ex: Retrait"
                                required type="text"  value="{{ $agence->nom }}"/>
                        </div>
                    </div>
                    ...
                    <div class="relative">
                        ...
                        <div class="relative group">
                            <input class="sunken-input w-full pl-12 pr-4 py-3.5 rounded-lg text-on-surface font-medium"
                                id="adresse" name="adresse" placeholder="Ex: 123 Rue de la Paix"
                                required type="text" value="{{ $agence->adresse }}" />
                        </div>
                    </div>
                </div>
            </section>
            ...
            <div class="flex flex-col md:flex-row gap-4 pt-6">
                <button
                 ...
                 <button type="submit" onclick="window.location.href='{{ route('liste_agences') }}'"
