    ...
    <div class="bg-surface-container-lowest rounded-xl kinetic-depth overflow-hidden">
        ...
        <div
            ...
            <div>
                ...
                <h1 class="text-4xl font-extrabold text-on-surface tracking-tight leading-none mb-4">Liste des
                    ...
                    <p class="text-on-surface-variant max-w-md text-lg">
                     @if (auth()->check() && auth()->user()->role === 'admin')
                        ...
                        @else
                        Découvrez les agences  des entreprises inscrites sur la plateforme.
                    @endif
                    </p>
             ...
             <button onclick="window.location.href='{{ route ('ajouter_agence') }}'" class="signature-glow text-white px-8 py-4 rounded-xl flex items-center justify-center gap-3 font-bold transition-all duration-300 hover:scale-[1.03] active:scale-95 shadow-lg shadow-orange-500/20 group">
                        ...
                        <tr class="hover:bg-surface-container-low/40 transition-colors group">
                            ...
                            <td class="px-8 py-6">
                                <div class="flex items-center gap-4">
                                    ...
                                    <span class="font-bold text-on-surface">{{ $agence->nom }}</span>
                                </div>
                            </td>
                            ...
                            <td class="px-8 py-6 text-right">
                                <div
                                    ...
                                    <button
                                        onclick="window.location.href='{{ route('formulaire_agence',['id_agence'=>$agence->id]) }}'"
                                     ...
                                     <form action="{{route('supprimer_agence',['id_agence'=>$agence->id])}}" method="POST">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit"
